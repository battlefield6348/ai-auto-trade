...
本功能目標：  
在「每日資料已成功寫入」的前提下，對 BTC/USDT（預留未來多交易對）進行標準化的批次分析計算，產生結構化的分析結果，供後續：
...
- 針對「日 K」與「基本資料」，進行每日一次的批次分析：
  ...
  - 產生交易對的當日分析結果與標籤（tags）
  - 產生可排序、可篩選的「分數 / 排名」基礎欄位
- 支援交易對批次分析，以及指定交易對重跑分析
- 分析結果的統一資料結構與儲存
- 分析任務的排程與錯誤處理
...
- 「BTC/USDT 日 K 取得（Data Ingestion）」已正常運作，且每日例行抓取已完成：
  - 目標交易日之日 K 資料已寫入儲存層
  - 交易對基本資料可查詢
- 可透過統一介面查詢：
  - 單檔交易對在指定日期區間的日 K 序列
...
1. 每個交易日晚上，系統自動對交易對交易對跑一次分析，產出完整的「當日分析結果快照」。
2. 針對某些交易對因資料修正或補抓，需要重跑該交易對在某些日期的分析結果。
...
對於每檔交易對，在進行「某一交易日 D」的分析時，最少需要以下資料：

1. 該交易對在日期 D 的日 K 資料：
   - 開、高、低、收、成交量、成交金額等
2. 該交易對在 D 之前的歷史日 K 序列（用於計算多日指標）：
...
> 若輸入資料不完整或不符合基本驗證規則，則該交易對該日的分析結果需標記為失敗並記錄原因。

### 5.2 分析輸出（分析結果資料結構）

每檔交易對 × 每個交易日，需產生一筆「分析結果」資料，包含：
...
2. 檢查該日 Data Ingestion 任務狀態：
   ...
   - 若為部分成功 → 照常執行，但對於缺失交易對會產生失敗記錄。
3. 取得當日交易對清單（可過濾下市、停牌等狀態）。
4. 依交易對為單位進行分析計算：
...
1. 以「日期」為主：
   - 針對某一交易日，重新跑交易對或部份交易對分析。
2. 以「交易對 × 日期區間」為主：
   - 例如：指定某檔交易對在某段日期重算分析結果（運維修補用）。
...
- 能支援以下查詢模式：
  1. 依「日期」查詢當日交易對或某市場的分析結果。
  2. 依「交易對代碼 + 日期區間」查詢該股歷史分析結果。
  3. 依特定條件篩選：
     - 產業別
     - 市場別
     - 分數區間
...
3. 查詢特定標籤組合：
   - 例如：取得「短期強勢」且「量能放大」且「產業為半導體」的交易對清單。
...
- 目標：在合理硬體資源下，對交易對交易對（上市+上櫃）完成一日分析的時間建議不超過 30 分鐘。
- 可採分批、併發等方式達成，但不得影響資料庫與其他核心服務的穩定性。

### 10.2 可靠性

- 任務不可默默失敗，需有紀錄可查。
- 若部分交易對無法分析，不得影響其他交易對的分析結果寫入。
