...
- 「BTC/USDT 日 K 取得（Data Ingestion）」  
...
1. 使用者在前端選擇日期與排序方式，取得「當日強勢交易對排行榜」（目前僅 BTC/USDT）。
2. 系統每日產生一份「量能放大交易對清單」檔案，用於後續策略或通知。
3. 研究人員查詢單一交易對在過去 120 個交易日的分析結果，用於畫圖或做回顧分析。
4. 工程師在除錯時，查詢特定交易對在某日的「分析輸入 + 分析輸出」摘要。
