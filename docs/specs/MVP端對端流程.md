...
- `身分驗證與授權.md`：登入、Token 驗證、最小角色/權限檢查。  
- `資料擷取流程.md`：單日日 K 抓取與寫入（手動觸發）。  
- `日批次分析.md`：單日分析，最小指標集合（漲跌幅、近 5 日報酬、成交量放大倍率、簡易 score）。  
- `查詢與匯出.md`：查詢某交易日全市場分析結果。  
- `選股篩選器.md`：固定規則的「強勢股」 API（不實作通用條件模型）。
