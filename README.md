...
1. 啟動
   ...
   - 管線/分析面板：瀏覽器開 `http://localhost:8080/`。
   - 回測面板：瀏覽器開 `http://localhost:8080/backtest.html`。
2. 登入
...
3. 管線/分析（日常操作）
   ...
   - 排程/任務歷史：首頁顯示下一次排程、最近一次任務摘要；歷史列表可展開查看失敗明細、耗時、錯誤訊息。
   - 單日操作：可選日期執行「擷取並分析」或「僅分析」，結果回饋區會顯示耗時、成功/失敗與錯誤明細。
   - 分析摘要/圖表：支援切換交易對、查看近期分析列表與折線圖。
4. 回測面板（`/backtest.html`）
   - 提供回測表單（交易對、起訖日、權重、門檻、Horizons 與條件開關），執行後回傳事件列表、總分/收盤線圖與 d5 forward return 柱狀圖。
   - 可儲存/載入個人預設（`/api/analysis/backtest/preset`），方便快速重跑常用設定。
4. 查詢與走勢
